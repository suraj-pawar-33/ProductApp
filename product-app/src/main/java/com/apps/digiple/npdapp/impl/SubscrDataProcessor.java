...
import com.apps.digiple.npdapp.view.subscription.SubscrDataEditPanel;
import com.apps.digiple.npdapp.view.subscription.SubscrTablePanel;
...
public class SubscrDataProcessor extends AbstractDataProcessor {
	private static final Logger LOGGER = Logger.getLogger(SubscrDataProcessor.class);
	...
	public SubscrDataProcessor() {
		proSelPanel = new ProSelectionPanel(getProSelEventHandler());
		tablePanel = new SubscrTablePanel(data, getMouseEventHandler());
		editPanel = new SubscrDataEditPanel(proSelPanel, tablePanel);
	}
	...
	public void loadDBData() {
		...
		try {
			...
			LOGGER.error("loadDBData > " + e.getLocalizedMessage());
		}
	}
	...
	public void setTablePanel(TablePanel tablePanel) {
		this.tablePanel = (SubscrTablePanel) tablePanel;
	}
	...
	public void setEditPanel(DataEditPanel editPanel) {
		this.editPanel = (SubscrDataEditPanel) editPanel;
	}
		...
		try {
			...
			for (int i = 0; i < property.length; i++) {
				...
				case OrderModel.DATE:
					((SubscrDataEditPanel) editPanel).setSubMonth(properties.get(property[i].getDisplayName()));
					break;
				case OrderModel.BILL_NUM:
					((SubscrDataEditPanel) editPanel).setBillNumber(properties.get(property[i].getDisplayName()));
					break;
				case OrderModel.TYPE:
					((SubscrDataEditPanel) editPanel).setOrderType(properties.get(property[i].getDisplayName()));
					break;
				case OrderModel.BANK:
					((SubscrDataEditPanel) editPanel).setBankName(properties.get(property[i].getDisplayName()));
					break;
				case OrderModel.AMOUNT:
					((SubscrDataEditPanel) editPanel).setTotalAmount(properties.get(property[i].getDisplayName()));
					break;
				case OrderModel.STATUS:
					((SubscrDataEditPanel) editPanel).setStatus(properties.get(property[i].getDisplayName()));
					break;
				default:
					break;
				}
