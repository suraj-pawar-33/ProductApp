...
public class BankDataEditPanel extends DataEditPanel {
	private static final Logger LOGGER = Logger.getLogger(BankDataEditPanel.class);
	private TextField nameText;
	private TextField brNameText;
	private TextField shortNameText;
	private AddressDataEditPanel addEdit;
	
	public BankDataEditPanel(BankAddDataEditPanel addEdit, TablePanel tablePanel) {
		super(tablePanel);
	...
	private GridPane getBankPane() {
		...
		Text branchLable = CustLabel.BK_BRANCH.getLebel();
...
//						DBHelper.saveBankAddressQuery(getCurrentAddress(), getObjectKey());
						...
						LOGGER.error("Bank data is not updated. KEY:" + getObjectKey() + " Reason : " + e.getLocalizedMessage());
						return;
					}
					showConfAlert("Success",
							"Bank data is updated " + getObjectKey());
					LOGGER.info("Bank data is updated KEY:" + getObjectKey());
				}
			}
		};
	}
			...
			public void handle(MouseEvent arg0) {
				if (notValidBankData()) {
					...
					try {
						...
						showConfAlert("Success", "New bank is Added " + bankKey);
						LOGGER.info("New bank is Added KEY:" + bankKey);
					} catch (Exception e) {
						showErrAlert("Error", "Bank is not saved.//n Reason : " + e.getLocalizedMessage());
						LOGGER.error("Bank is not saved.  KEY:" + bankKey + " Reason : " + e.getLocalizedMessage());
					}
				}
			}
		};
		...
		return new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent arg0) {
				...
				try {
					...
					LOGGER.error("Bank is not deleted. KEY:" + getObjectKey()+ ", Reason : " + e.getLocalizedMessage());
				}
				showConfAlert("Success", "Bank dats is deleted " + getObjectKey());
				LOGGER.info("Bank dats is deleted KEY:" + getObjectKey());
				clearAllText();
