...
public class OrderDataEditPanel extends DataEditPanel {
	private static final Logger LOGGER = Logger.getLogger(OrderDataEditPanel.class);
	...
	public void setOrderDate(String value) {
		orderPlacedDate.setValue(getLocalDate(value));
	}
	...
	protected EventHandler<MouseEvent> saveEvent() {
		return new EventHandler<MouseEvent>() {
			...
			@Override
			public void handle(MouseEvent arg0) {
				if (StringUtils.isNullOrEmpty(getBankName()) || !StringUtils.isStrictlyNumeric(billNumberText.getText()) || StringUtils.isNullOrEmpty(getOrderType())
						|| getOrderDate() == null || !StringUtils.isStrictlyNumeric(getTotalAmount())
						|| StringUtils.isNullOrEmpty(getStatus())) {
					showErrAlert("Data Error!",
							"Bank Name, Type are required and Total amount, bill number need to be numeric value");
					return;
				} else {
					OrderModel order = new OrderModel(0, getOrderDate().toString(), getBillNumber(), getBankName(),
		...
		return new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent arg0) {
				if (StringUtils.isNullOrEmpty(getBankName()) || StringUtils.isNullOrEmpty(getOrderType())
					...
					OrderModel order = new OrderModel(getObjectKey(), getOrderDate().toString(), getBillNumber(),
	...
	private HashMap<String, Object> getPrintingParams() {
		...
		map.put(OrderModel.DATE, getOrderDate());
