...
public class ProSelectionPanel extends TablePanel {
	private static final Logger LOGGER = Logger.getLogger(ProSelectionPanel.class);
	...
	protected void loadColumns() {
		...
		qntyCol.setCellValueFactory(cellData -> cellData.getValue().getQntyEntryProperty());
	...
	public void selectItems(List<Order2ProductModel> products) {
		...
		for (Iterator<Order2ProductModel> iterator = products.iterator(); iterator.hasNext();) {
			Order2ProductModel order2ProductModel = iterator.next();
	...
	@SuppressWarnings("unchecked")
	public List<Order2ProductModel> getSelectedProducts() {
		...
		for (Iterator<ProductModel> iterator = items.iterator(); iterator.hasNext();) {
			...
			if (!productModel.getQntyEntry().equals("0")) {
				list.add(new Order2ProductModel(0, getOrderKey(), productModel.getProductKey(), Integer.valueOf(productModel.getQntyEntry()), Integer.valueOf(productModel.getProductCost())));
			}
		}
		return list;
	}
